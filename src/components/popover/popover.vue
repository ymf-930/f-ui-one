...
import FPopper, { defaultProps, Theme } from '../popper'
...
const _hoist = { key: 0, class: 'fei-popover__title', role: 'title' }
const _content = { key: 1, class: 'fei-popover__body', role: 'body' }
export default {
  name: 'FPopover',
  components: {
    FPopper,
  ...
  setup(props, ctx) {
    if (process.env.NODE_ENV !== 'production' && props.visible && !ctx.slots.reference) {
      throwWarn('FPopover', ' You cannot init popover without given reference')
    }
  ...
  render() {
    ...
    const kls = [
      this.content ? 'fei-popover--plain' : '',
      'fei-popover',
      popperClass,
    ].join(' ')
