<template>
  <div :class="wrapClasses" :style="wrapStyles" ref="containerRef">
    <div :class="classes" v-loading:[loadingText]="loading">
      <div :class="['fei-table-header']" v-if="showHeader" ref="headerRef" @mousewheel="handleMouseWheel">
        <table-head
          prefix-cls="fei-table"
      ...
      <div :class="['fei-table-body']" ref="bodyRef" :style="bodyStyle" @scroll="handleBodyScroll"
      ...
      <div
        :class="['fei-table-tip']" :style="bodyStyle" @scroll="handleBodyScroll"
      ...
      <div :class="['fei-table-fixed']" :style="fixedTableStyle" v-if="isLeftFixed">
        ...
        <div :class="['fei-table-fixed-body']" :style="fixedBodyStyle" ref="fixedBodyRef"
      ...
      <div :class="['fei-table-fixed-right']" :style="fixedRightTableStyle" v-if="isRightFixed">
        <div :class="fixedHeaderClasses" v-if="showHeader">
          <table-head
            fixed="right"
            prefix-cls="fei-table"
        ...
        <div :class="['fei-table-fixed-body']" :style="fixedBodyStyle" ref="fixedRightBodyRef"
             @mousewheel="handleFixedMousewheel" @DOMMouseScroll="handleFixedMousewheel">
          <table-body
            fixed="right"
            :draggable="draggable"
      ...
      <div :class="['fei-table-fixed-right-header']" :style="fixedRightHeaderStyle" v-if="isRightFixed"></div>
    </div>
  </div>
</template>
...
import FEmpty from '../empty/empty'
import { generateId } from '../../utils/util-helper'

const prefixCls = 'fei-table'
...
export default {
  name: 'FTable',
  components: { FEmpty, TableBody, TableHead },
  props: {
    data: {
      type: Array,
      default() {
  ...
  setup(props, { emit, slots }) {
    ...
    function fixedBody() {
      ...
      if (!tbody || !props.data || props.data.length === 0) {
        ...
        if (showVerticalScrollBar.value) {
          bodyEl.classList.add('fei-table-overflowY')
        } else {
          bodyEl.classList.remove('fei-table-overflowY')
        }
        if (showHorizontalScrollBar.value) {
          bodyEl.classList.add('fei-table-overflowX')
        } else {
          bodyEl.classList.remove('fei-table-overflowX')
        }
      }
    }
    ...
    function setSort() {
      ...
      const el = tbodyRef.value.$el.querySelector('.fei-table-tbody')
      if (!el) {
        console.error('dom width class fei-table-tbody is not find')
        return
      }
      sortInstance = Sortable.create(el, {
        animation: 150,
        ghostClass: 'fei-table-ghost-class',
