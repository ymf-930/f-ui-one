<template>
  ...
  >
    <f-popper
      ...
      :popper-class="`fei-icon-select__popper ${popperClass}`"
      manual-mode
      theme="light"
      pure
      trigger="click"
    ...
    >
      <template #trigger>
        <div class="select-trigger">
          ...
          >
            <template #prepend>
              <f-button style="width: 40px;padding: 0;">
                <f-icon :name="selectedLabel" size="16"></f-icon>
              </f-button>
            </template>
            <template #suffix>
      ...
      <template #default>
        <div class="fei-icon-select-panel__query">
          <f-input v-model="query" placeholder="输入图标名称搜索" size="small" clearable></f-input>
        </div>
        <f-scrollbar
          v-show="visible && showIcons.length"
          ref="scrollbar"
          tag="ul"
          wrap-class="fei-icon-select-panel__wrap"
        ...
        <f-empty v-show="showIcons.length===0">没有匹配图标</f-empty>
      </template>
    </f-popper>
  </div>
</template>
...
export default {
  name: 'FIconSelect',
  components: { FEmpty, FIcon, FButton, FScrollbar, FInput, FPopper },
