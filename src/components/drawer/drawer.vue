        ...
        <transition :name="'move-' + placement" @after-leave="wrapShow = false">
          <div :class="classes" :style="mainStyles" v-show="visible">
            <div :class="contentClasses" ref="content">
              <a class="fei-drawer-close" v-if="showClose" @click="close">
                <slot name="close">
                  <i class="f-iconfont f-icon-close"></i>
                </slot>
              </a>
              <div class="fei-drawer-header" v-if="$slots.header || title">
                <slot name="header">
                  <div class="fei-drawer-header-inner">{{ title }}</div>
                </slot>
              </div>
              <div class="fei-drawer-body" :style="styles">
                <slot></slot>
              </div>
              <div class="fei-drawer-footer" v-if="$slots.footer">
            ...
            <div
              class="fei-drawer-drag"
              :class="{ 'fei-drawer-drag-left': placement === 'left' }"
              v-if="draggable"
              @mousedown="handleTriggerMousedown"
            >
              <slot name="trigger">
                <div class="fei-drawer-drag-move-trigger">
                  <div class="fei-drawer-drag-move-trigger-point">
                    <i></i><i></i><i></i><i></i><i></i>
                  </div>
                </div>
              </slot>
...
export default {
  ...
  methods: {
    ...
    handleWrapClick(event) {
      // use indexOf,do not use === ,because fei-modal-wrap can have other custom className
