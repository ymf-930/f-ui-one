<template>
  <div :class="['fei-cascader-panel', border && 'is-bordered']" @keydown="handleKeyDown">
    <f-cascader-menu
...
import FCascaderMenu from './menu.vue'
  ...
  setup(props, { emit, slots }) {
    ...
    const scrollToExpandingNode = () => {
      ...
      menuList.value.forEach((menu) => {
        ...
        if (menuElement) {
          const container = menuElement.querySelector('.fei-scrollbar__wrap')
          const activeNode =
            menuElement.querySelector('.fei-cascader-node.is-active') ||
            menuElement.querySelector('.fei-cascader-node.in-active-path')
      ...
      switch (code) {
        ...
        case EVENT_CODE.left:
          ...
          const expandedNode = preMenu?.$el.querySelector(
            '.fei-cascader-node[aria-expanded="true"]',
          )
        ...
        case EVENT_CODE.right:
          ...
          // eslint-disable-next-line no-case-declarations
          const firstNode = nextMenu?.$el.querySelector('.fei-cascader-node[tabindex="-1"]')
          focusNode(firstNode)
          break
