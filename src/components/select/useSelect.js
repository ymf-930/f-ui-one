...
export const useSelect = (props, states, ctx) => {
  ...
  const selectDisabled = computed(() => props.disabled || FForm.disabled)
  ...
  const selectSize = computed(() => props.size || FForm.size || FFormItem.size)
  ...
  const scrollToOption = option => {
    ...
    if (popper.value && target) {
      const menu = popper.value?.popperRef?.querySelector?.('.fei-select-dropdown__wrap')
      if (menu) {
