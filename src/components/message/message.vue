<template>
  <transition name="fei-message-fade">
    <div
      v-show="visible"
      :id="id"
      :class="[
        'fei-message',
        type && !iconClass ? `fei-message--${type}` : '',
    ...
    >
      ...
      <slot>
        <p v-if="!useHTML" class="fei-message__content">
          {{ message }}
        </p>
        <p v-else class="fei-message__content" v-html="message"></p>
      </slot>
      <div
        v-if="showClose"
        class="f-iconfont f-icon-close"
        @click.stop="close"
      ></div>
    </div>
  </transition>
...
export default {
  ...
  computed: {
    typeClass() {
      ...
      return type && TypeMap[type] ? `fei-message__icon f-iconfont f-icon-${TypeMap[type]} is-${type}` : ''
    },
    customStyle() {
      return {
        top: `${this.offset}px`,
