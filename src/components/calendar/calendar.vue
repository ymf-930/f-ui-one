<template>
  <div class="fei-calendar" :class="{'card':mini}">
    <div class="fei-calendar-header">
      <template v-if="mini">
        <span>{{ headerTitle }}</span>
        <f-button-group>
          <f-button size="mini" @click="handlePrevMonth" icon="doubleleft"></f-button>
          <f-button size="mini" @click="handleToday">今天</f-button>
          <f-button size="mini" @click="handleNextMonth" icon="doubleright"></f-button>
        </f-button-group>
      </template>
      <template v-else>
        <div class="fei-calendar-header-title">
          {{ headerTitle }}
        </div>
        <f-button-group>
          <f-button size="mini" @click="handlePrevMonth">上个月</f-button>
          <f-button size="mini" @click="handleToday">今天</f-button>
          <f-button size="mini" @click="handleNextMonth">下个月</f-button>
        </f-button-group>
      </template>
    </div>
    <ul class="fei-calendar-week">
      <li v-for="(item, index) in ['一', '二', '三', '四', '五', '六', '日']" :key="index" class="fei-calendar-week-item">
        {{ item }}
      </li>
    </ul>
    <ul class="fei-calendar-body" :style="bodyStyle">
      <li v-for="(item, index) in visibleCalendar" :key="index"
          class="fei-calendar-date-view" :style="dateViewStyle"
