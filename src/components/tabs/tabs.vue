<template>
  <div class="fei-tabs-wrapper" :class="[type,{'closable':closable}]" ref="rootRef">
    <scroll-pane class="nav-wrapper" ref="scrollPaneRef">
      <div class="tab-list">
        ...
        <i v-if="tab.icon" class="custom-icon" :class="['f-iconfont',`f-icon-${tab.icon}`]"></i>
        {{ tab.title }}
        <i v-if="type!=='default'&& closable && !tab.noClose" class="f-iconfont f-icon-close"
