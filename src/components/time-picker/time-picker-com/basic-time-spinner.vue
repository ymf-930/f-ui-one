<template>
  <div class="fei-time-spinner" :class="{ 'has-seconds': showSeconds }">
    <template v-if="!arrowControl">
      <f-scrollbar
        ref="listHoursRef"
        class="fei-time-spinner__wrapper"
        wrap-style="max-height: inherit;"
        view-class="fei-time-spinner__list"
        noresize
        tag="ul"
    ...
    <template v-if="arrowControl">
      <div
        ...
        :key="item"
        class="fei-time-spinner__wrapper is-arrow"
        @mouseenter="emitSelectRange(item)"
      >
        <i @click="onDecreaseClick" class="fei-time-spinner__arrow f-iconfont f-icon-up"></i>
        <i @click="onIncreaseClick" class="fei-time-spinner__arrow f-iconfont f-icon-down"></i>
        <ul class="fei-time-spinner__list">
          <li
...
export default {
  ...
  setup(props, ctx) {
    ...
    const adjustSpinner = (type, value) => {
      ...
      if (el.value) {
        el.value.$el.querySelector('.fei-scrollbar__wrap').scrollTop = Math.max(
          0,
    ...
    const handleScroll = (type) => {
      ...
      const scrollTop = listRefsMap[type].value.$el.querySelector('.fei-scrollbar__wrap').scrollTop
    ...
    const bindScrollEvent = () => {
      const bindFuntion = (type) => {
        if (listRefsMap[type].value) {
          listRefsMap[type].value.$el.querySelector('.fei-scrollbar__wrap').onscroll = () => {
