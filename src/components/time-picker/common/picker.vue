<template>
  <f-popper
    ...
    :popper-class="`fei-picker__popper ${popperClass}`"
  ...
  >
    <template #trigger>
      <f-input
        ...
        class="fei-date-editor"
        :class="'fei-date-editor--' + type"
      ...
      >
        ...
        <template #suffix>
          <i
            class="f-iconfont"
            :class="[showClose ? 'f-icon-' + clearIcon : '']"
      ...
      <div
        v-else
        v-click-outside:[popperPaneRef]="onClickOutside"
        class="fei-date-editor fei-range-editor fei-input"
        :class="[
          'fei-date-editor--' + type,
          pickerSize ? `fei-range-editor--${ pickerSize }` : '',
        ...
        @keydown="handleKeydown"
      >
        <i :class="['fei-input__icon f-iconfont', 'fei-range__icon', triggerClass]"></i>
        <input
          autocomplete="off"
          :name="name && name[0]"
          ...
          class="fei-range-input"
          @input="handleStartInput"
          @change="handleStartChange"
          @focus="handleFocus"
        >
        <slot name="range-separator">
          <span class="fei-range-separator">{{ rangeSeparator }}</span>
        </slot>
        <input
          autocomplete="off"
          :name="name && name[1]"
        ...
        >
        <i
          class="fei-input__icon f-iconfont"
          :class="[showClose ? 'f-icon-' + clearIcon : '']"
          @click="onClearIconClick"
        >
        </i>
...
export default {
  ...
  setup(props, ctx) {
    ...
    const pickerDisabled = computed(() => {
      return props.disabled || FForm.disabled
    })
    ...
    const triggerClass = computed(() => {
      return props.prefixIcon || (isTimeLikePicker.value ? 'f-icon-time-circle' : 'f-icon-calendar')
    })
    ...
    const pickerSize = computed(() => {
      return props.size || FFormItem.size
    })
