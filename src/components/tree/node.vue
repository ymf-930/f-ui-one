    ...
    <li v-show="data.visible">
      ...
      >
        <span :class="arrowClasses">
          <i v-if="showArrow" class="f-iconfont f-icon-caret-right" @click="handleExpand"></i>
          <template v-if="!isLeaf">
            <i v-if="showLoading" class="f-iconfont f-icon-loading fei-load-loop"></i>
          </template>
        </span>
        <f-checkbox
          v-if="showCheckbox"
        ...
        <template v-else>
          ...
          <span v-else :class="titleClasses" :title="data[titleKey]" @click="handleSelect">
            <i v-if="data.icon" :class="`f-iconfont f-icon-${data.icon}`"></i>{{ data[titleKey] }}
          </span>
        </template>
      </div>
      <collapse-transition>
        <div class="fei-tree-node-children" v-show="data.expand">
          <tree-node
...
const prefixCls = 'fei-tree'

export default {
  name: 'TreeNode',
  components: { CollapseTransition, Render, FCheckbox },
  props: {
    data: {
      type: Object,
      default: () => {
  ...
  setup(props) {
    const TreeInstance = inject('FTreeRoot', {})
    const node$ = ref(null)
