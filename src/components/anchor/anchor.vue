<template>
  <div class="fei-anchor-wrapper" ref="container">
    <div class="fei-anchor">
      <div class="fei-anchor-ink">
        <f-icon v-if="icon" :name="icon" :style="iconStyle" :color="activeColorStr"></f-icon>
        <span
          v-show="!icon && showInk"
          class="fei-anchor-ink-ball"
          :style="{ borderColor: activeColorStr, top: `${inkTop}px` }"
        ></span>

        <span
          v-show="!icon && !showInk"
          class="fei-anchor-ink-line"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    provide('FAnchor', { props, data, linksRef })
